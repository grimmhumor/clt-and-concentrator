...
export class Concentrator {
    ...
    /* -------------------------------------------- */
    static _onReady() {
        const enableConcentrator = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.enable);
        
        if (!game.user.isGM || !enableConcentrator) return;

        const conditionName = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.conditionName);

        const conditionExists = !!EnhancedConditions.getCondition(conditionName);

        if (!conditionExists) {
            console.log(`${game.i18n.localize(`CLT.CONCENTRATOR.MessagePrefix`)} ${game.i18n.localize(`CLT.CONCENTRATOR.NoMatchingCondition`)}`);
            return ui.notifications.warn(game.i18n.localize(`CLT.CONCENTRATOR.NoMatchingCondition`));
        }
    }
    ...
    static async _onRenderChatMessage(app, html, data) {
        ...
        const autoConcentrate = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.autoConcentrate);
        const concentrateFlag = app.getFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.chatMessage);
        ...
        const spell = {
            id: item?.id ?? "",
            name: item?.name ?? game.i18n.localize(`CLT.CONCENTRATOR.UnknownSpell`)
        };

        let sendMessage = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyConcentration);
        ...
        if (isAlreadyConcentrating && Concentrator._shouldSendMessage("double")) {
            ...
            sendMessage = BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentration.none;
        }
        ...
        return app.setFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.chatMessage, true);
    }
    ...
    static _onPreUpdateActor(actor, update, options, userId) {
        ...
        const newTempHealth = getProperty(update, `system.${Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.healthAttribute)}.temp`);
        const oldTempHealth = getProperty(actor, `system.${Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.healthAttribute)}.temp`);
        const tempDamageTaken = Concentrator._wasDamageTaken(newTempHealth, oldTempHealth);
        
        const newHealth = getProperty(update, `system.${Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.healthAttribute)}.value`);
        const oldHealth = getProperty(actor, `system.${Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.healthAttribute)}.value`);
        ...
        if (tempDamageTaken || damageTaken) {
            options[BUTLER.NAME][BUTLER.FLAGS.concentrator.damageTaken] = true;
            options[BUTLER.NAME][BUTLER.FLAGS.concentrator.damageAmount] = tempDamageTaken ? Concentrator._calculateDamage(newTempHealth, oldTempHealth) : Concentrator._calculateDamage(newHealth, oldHealth);
            options[BUTLER.NAME][BUTLER.FLAGS.concentrator.isDead] = newHealth <= 0;
        }

        setProperty(options, `CLT.${BUTLER.FLAGS.concentrator.updateProcessed}`, false);
        return true;
    }
    ...
    static _onUpdateActor(actor, update, options, userId){
        const damageTaken = getProperty(options, `CLT.${BUTLER.FLAGS.concentrator.damageTaken}`);
        const updateProcessed = getProperty(options, `CLT.${BUTLER.FLAGS.concentrator.updateProcessed}`);
        ...
        setProperty(options, `CLT.${BUTLER.FLAGS.concentrator.updateProcessed}`, true);
    ...
    static _onPreUpdateToken(token, update, options, userId){
        ...
        const newTempHealth = getProperty(update, `actorData.system.${Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.healthAttribute)}.temp`);
        const oldTempHealth = getProperty(token, `actor.system.${Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.healthAttribute)}.temp`);
        
        const tempDamageTaken = Concentrator._wasDamageTaken(newTempHealth, oldTempHealth);

        const newHealth = getProperty(update, `actorData.system.${Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.healthAttribute)}.value`);
        const oldHealth = getProperty(token, `actor.system.${Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.healthAttribute)}.value`);
        
        const damageTaken = Concentrator._wasDamageTaken(newHealth, oldHealth);

        if (tempDamageTaken || damageTaken) {
            const cubOption = options[BUTLER.NAME] = options[BUTLER.NAME] ?? {};
            cubOption[BUTLER.FLAGS.concentrator.damageTaken] = true;
            cubOption[BUTLER.FLAGS.concentrator.damageAmount] = tempDamageTaken ? Concentrator._calculateDamage(newTempHealth, oldTempHealth) : Concentrator._calculateDamage(newHealth, oldHealth);
            cubOption[BUTLER.FLAGS.concentrator.isDead] = newHealth <= 0;
        }
    ...
    static _onDeleteActiveEffect(effect, options, userId) {
        ...
        const concentrationSpellFlag = actor.getFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.concentrationSpell);

        if (concentrationSpellFlag?.status !== BUTLER.DEFAULT_CONFIG.concentrator.concentrationStatuses.active) return;

        const conditionIdFlag = effect.getFlag(BUTLER.NAME, BUTLER.FLAGS.enhancedConditions.conditionId);

        if (!conditionIdFlag) return;

        const condition = Sidekick.getSetting(BUTLER.SETTING_KEYS.enhancedConditions.map).find(c => c.id === conditionIdFlag);
        const concentrationConditionName = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.conditionName);

        if (!condition || condition?.name !== concentrationConditionName) return;

        const sendMessage = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyEndConcentration);
    ...
    static _onRenderActorSheet(app, html, data) {
        ...
        const concentrationFlag = actor?.getFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.concentrationSpell);
    ...
    static _processDamage(entity, options) {
        ...
        const displayPrompt = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.prompt);

        if (!entity || !isConcentrating || !displayPrompt) return;

        const damageAmount = getProperty(options, `CLT.${BUTLER.FLAGS.concentrator.damageAmount}`);
        const isDead = getProperty(options, `CLT.${BUTLER.FLAGS.concentrator.isDead}`);
    ...
    static async _distributePrompts(uuid, userIds, dc){
        ...
        const requestData = {
            gadget: BUTLER.GADGETS.concentrator.name,
            action: "prompt",
        ...
        game.socket.emit(`module.CLT`, requestData);
    }
    ...
    static async _displayPrompt(uuid, userId, dc){
        ...
        const spell = actor.getFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.concentrationSpell);
        const spellName = spell?.name ?? game.i18n.localize(`CLT.CONCENTRATOR.UnknownSpell`);

        new Dialog({
            title: game.i18n.localize(`CLT.CONCENTRATOR.Prompts.Check.Title`),
            content: game.i18n.format(`CLT.CONCENTRATOR.Prompts.Check.Content`, {actorName: actor.name, spellName}),
            buttons: {
                yes: {
    ...
    static _processConcentrationCheck(event, actor, dc) {
        const ability = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.concentrationAttribute);
        actor.rollAbilitySave(ability);
        game.socket.emit(`module.CLT`, {
            gadget: BUTLER.GADGETS.concentrator.name,
        ...
        Hooks.once("createChatMessage", (message, options, userId) => {
            ...
            const autoEndConcentration = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.autoEndConcentration);
    ...
    static _cancelPrompt(userId) {
        ...
        const dialog = Object.values(ui.windows)?.find(w => w.title === game.i18n.localize(`CLT.CONCENTRATOR.Prompts.Check.Title`));

        if (!dialog) return;

        dialog.close();
        ui.notifications.notify(`${game.i18n.localize(`CLT.SHORT_BUTLER.NAME`)} | ${game.i18n.localize(`CLT.CONCENTRATOR.Prompts.Check.ClosedByOther`)}`);
    }
     ...
     * @param {*} damage
     */
    static _displayChat(type, entity, {dc=0, newSpell={name: game.i18n.localize(`CLT.CONCENTRATOR.UnknownSpell`), id: ""}}={}){
        ...
        const hideNpcConcentration = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.hideNpcConcentration);
        ...
        switch (type) {
            case "start":
                content =  game.i18n.format(`CLT.CONCENTRATOR.Messages.StartConcentration`, {entityName: entity.name, spellName});
                isWhisper = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyConcentration) === Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentration, BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentration.gm);
                break;

            case "check":
                content = game.i18n.format(`CLT.CONCENTRATOR.Messages.ConcentrationTested`, {entityName: entity.name, dc, spellName});
                isWhisper = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyConcentrationCheck) === Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentrationCheck, BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentrationCheck.gm);
                break;

            case "dead":
                content = game.i18n.format(`CLT.CONCENTRATOR.Messages.Incapacitated`, {entityName: entity.name, spellName});
                isWhisper = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyEndConcentration) === Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyEndConcentration, BUTLER.DEFAULT_CONFIG.concentrator.notifyEndConcentration.gm);
                break;

            case "double":
                content =  game.i18n.format(`CLT.CONCENTRATOR.Messages.DoubleConcentration`, {entityName: entity.name, oldSpellName: spellName, newSpellName: newSpell.name})
                isWhisper = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyDouble) === Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyDouble, BUTLER.DEFAULT_CONFIG.concentrator.notifyDouble.gm);
                break;

            case "end":
                content = game.i18n.format(`CLT.CONCENTRATOR.Messages.EndConcentration`, {entityName: entity.name, spellName})
                isWhisper = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyEndConcentration) === Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyEndConcentration, BUTLER.DEFAULT_CONFIG.concentrator.notifyEndConcentration.gm);
                break;
        ...
        const whisperRecipients = isWhisper ? Concentrator._getWhisperRecipients(entity, Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.messageVisibility, BUTLER.DEFAULT_CONFIG.concentrator.messageVisibility.gmOwner)) : [];
     ...
     */
    static async _startConcentration(entity, spell, conditionName, {sendMessage=BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentration.none}={}) {
        const isActor = entity instanceof Actor;
        const isToken = entity instanceof Token || entity instanceof TokenDocument;
        ...
        return actor.setFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.concentrationSpell, {
            id: spell.id,
            name: spell.name,
            status: BUTLER.DEFAULT_CONFIG.concentrator.concentrationStatuses.active
        });
    }
    ...
    static async _endConcentration(entity, {reason=null}={}) {
        ...
        const flag = actor.getFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.concentrationSpell);

        if (flag) {
            const flagUpdate = {status: BUTLER.DEFAULT_CONFIG.concentrator.concentrationStatuses.breaking};
            await actor.setFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.concentrationSpell, mergeObject(flag, flagUpdate));
        }
        ...
        return actor.unsetFlag(BUTLER.NAME, BUTLER.FLAGS.concentrator.concentrationSpell);
    }
    ...
    static _promptEnableEnhancedConditions() {
        const title = game.i18n.localize(`CLT.CONCENTRATOR.Prompts.EnableEnhancedConditions.Title`);
        const content = game.i18n.localize(`CLT.CONCENTRATOR.Prompts.EnableEnhancedConditions.Content`);
        new Dialog({
            title,
            content,
            buttons: {
                yes: {
                    ...
                    icon: `<i class="fas fa-check"></i>`,
                    callback: async e => {
                        await Sidekick.setSetting(BUTLER.SETTING_KEYS.enhancedConditions.enable, true, true);
    ...
    static _createCondition() {
        ...
        const enhancedConditions = Sidekick.getSetting(BUTLER.SETTING_KEYS.enhancedConditions.enable);
    ...
    static _shouldSendMessage(eventType) {
        ...
        switch (eventType) {
            case 'start':
                keyNone = Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentration, BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentration.none);
                setting = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyConcentration);
            ...
            case 'check':
                keyNone = Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentrationCheck, BUTLER.DEFAULT_CONFIG.concentrator.notifyConcentrationCheck.none);
                setting = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyConcentrationCheck);
                if ((setting && keyNone) && setting !== keyNone) {
                    shouldSend = true;
                }
                break;
            ...
            case 'double':
                keyNone = Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyDouble, BUTLER.DEFAULT_CONFIG.concentrator.notifyDouble.none)
                setting = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyDouble);
                if ((setting && keyNone) && setting !== keyNone) {
                    shouldSend = true;
                }
                break;
            ...
            case 'end':
                keyNone = Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.notifyEndConcentration, BUTLER.DEFAULT_CONFIG.concentrator.notifyEndConcentration.none)
                setting = Sidekick.getSetting(BUTLER.SETTING_KEYS.concentrator.notifyEndConcentration);
                if ((setting && keyNone) && setting !== keyNone) {
                    shouldSend = true;
                }
                break;
    ...
    static _getWhisperRecipients(entity, desiredVisibility) {
        ...
        const visibilityGmOwner = Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.messageVisibility, BUTLER.DEFAULT_CONFIG.concentrator.messageVisibility.gmOwner);
        const visibilityAll = Sidekick.getKeyByValue(BUTLER.DEFAULT_CONFIG.concentrator.messageVisibility, BUTLER.DEFAULT_CONFIG.concentrator.messageVisibility.all);
